    ...
    async def _retrieval_node(self, state: AgentState) -> Dict[str, Any]:
        ...
        if not query:
            return {
                ...
                + [AIMessage(content="Error: No query provided")],
            }
        ...
        try:
            ...
            query_response = await self.kb_repo.query(
                QueryRequest(query=query, top_k=5)
            )
            ...
            return {
                ...
                "messages": state.get("messages", [])
                + [AIMessage(content=f"Retrieved {len(retrieved_docs)} documents")],
            }
        except Exception as e:
            ...
            return {
                "error": str(e),
                "messages": state.get("messages", [])
                + [AIMessage(content=f"Error retrieving documents: {e}")],
            }
    ...
    async def _generation_node(self, state: AgentState) -> Dict[str, Any]:
        ...
        if not docs:
            return {
                ...
                "sources": [],
                "messages": state.get("messages", [])
                + [AIMessage(content="No relevant information found for this query.")],
            }
        ...
        try:
            ...
            return {
                "error": str(e),
                "messages": state.get("messages", [])
                + [AIMessage(content=f"Error generating answer: {e}")],
            }
    ...
    async def answer_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.workflow.ainvoke(
                state, config={"configurable": {"session_id": session_id}}
            )
