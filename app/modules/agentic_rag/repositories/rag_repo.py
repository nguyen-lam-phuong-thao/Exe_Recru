    ...
    async def generate(self, request: RAGRequest) -> RAGResponse:
        ...
        try:
            ...
            query_response = await self.kb_repo.query(
                QueryRequest(query=request.query, top_k=request.top_k)
            )
            ...
            print(f"RAGRepository: Generated response: {result}")
            usage = {
                "prompt_tokens": len(context) // 4
                + len(request.query) // 4,  # Approximation
                "completion_tokens": len(result["text"]) // 4,  # Approximation
                "total_tokens": (
                    len(context) + len(request.query) + len(result["text"])
                )
                // 4,  # Approximation
            }

            print(f"RAGRepository: Generated response for query '{request.query}'")
            return RAGResponse(answer=result["text"], sources=sources, usage=usage)
