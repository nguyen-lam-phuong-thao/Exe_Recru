...
class QuestionComposerRepo:
	...
	async def generate_questions(self, request: QuestionGenerationRequest) -> QuestionGenerationResponse:
		...
		try:
			...
			logger.info(f'⚙️ Workflow parameters - User profile completeness estimate: {len(user_profile_data)} fields, Previous questions: {len(previous_questions)}')
	...
	def get_service_info(self) -> Dict[str, Any]:
		...
		logger.info('ℹ️ Retrieving service information')
		...
		try:
			...
			logger.info(f'⚙️ Workflow info keys: {list(workflow_info.keys())}')
	...
	def _create_new_session(self, request: QuestionGenerationRequest, session_id: str):
		...
		logger.info(f'📊 Existing user data keys: {list((request.existing_user_data or {}).keys())}')
