...
class QuestionAgentRepository:
	...
	async def generate_initial_questions(
		...
		if knowledge_context.get('knowledge'):
			...
			if knowledge_context.get('sources'):
				sources_summary = f' (Sources: {len(knowledge_context["sources"])} found)'
	...
	async def evaluate_and_refine_questions(
		...
		for i, q in enumerate(questions):
			...
			refined_questions.append(GeneratedQuestion(id=f'{q.id}_refined', text=refined_text, category=q.category))
	...
	async def select_critical_questions(self, questions: List[GeneratedQuestion], count: int = 10) -> List[GeneratedQuestion]:
		"""
	...
	async def compose_questions(self, user_characteristics: UserCharacteristicsInput) -> List[GeneratedQuestion]:
		"""
		...
		try:
			user_char_summary = ', '.join([f'{k}: {v}' for k, v in user_characteristics.data.items()])
			derived_query: str = f'Generate insights and questions for career reflection based on these characteristics: {user_char_summary}'

			knowledge_context = await query_rag_system(derived_query)
			print(f'Retrieved knowledge context: {knowledge_context}')

			if not knowledge_context.get('knowledge') or 'Error from RAG' in knowledge_context.get('knowledge', ''):
				print(f'Warning: Knowledge base query returned no significant information or an error: {knowledge_context.get("knowledge")}')
		...
		try:
			initial_questions: List[GeneratedQuestion] = await self.generate_initial_questions(user_characteristics, knowledge_context)
			...
			for i in range(2):
				...
				current_refined_questions = await self.evaluate_and_refine_questions(refined_questions, user_characteristics, knowledge_context)
			...
			final_questions: List[GeneratedQuestion] = await self.select_critical_questions(refined_questions, count=10)
