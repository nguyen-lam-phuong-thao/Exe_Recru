...
class EducationItem(BaseModel):
	...
	graduation_date: Optional[str] = Field(None, description='Date of graduation or expected graduation (e.g., YYYY-MM, YYYY).')
	gpa: Optional[str] = Field(None, description='Grade Point Average or academic score (as string for flexibility).')
	relevant_courses: Optional[List[str]] = Field(default_factory=list, description='List of relevant courses taken.')
	description: Optional[str] = Field(None, description='Additional details, honors, thesis information, or activities.')
...
class WorkExperienceItem(BaseModel):
	...
	responsibilities_achievements: Optional[List[str]] = Field(default_factory=list, description='Key responsibilities, accomplishments, and projects.')
...
class SkillItem(BaseModel):
	...
	proficiency_level: Optional[str] = Field(None, description='Proficiency level (e.g., Beginner, Intermediate, Advanced, Expert).')
	category: Optional[str] = Field(None, description='Category of the skill (e.g., Programming Language, Software, Soft Skill, Framework).')
...
class ProjectItem(BaseModel):
	...
	description: Optional[str] = Field(None, description='Detailed description of the project, its purpose, and outcomes.')
	technologies_used: Optional[List[str]] = Field(default_factory=list, description='List of technologies, tools, or programming languages used.')
	role: Optional[str] = Field(None, description='Your role or contribution to the project.')
	project_url: Optional[str] = Field(None, description='URL to the project (e.g., GitHub repository, live demo, publication).')
...
class InferredCharacteristicItem(BaseModel):
	...
	characteristic_type: str = Field(..., description='Type of characteristic (e.g., Potential Role, Key Strength, Soft Skill, Technical Expertise, Area for Development).')
	statement: str = Field(..., description='The inferred statement or insight about the candidate.')
	evidence: Optional[List[str]] = Field(default_factory=list, description='Snippets or points from the CV that support this inference.')
...
class CVAnalysisResult(BaseModel):
	...
	processed_cv_text: Optional[str] = Field(None, description='The cleaned and structured text version of the CV used for analysis.')

	identified_sections: List[str] = Field(default_factory=list, description="List of section titles identified in the CV (e.g., 'Education', 'Work Experience').")

	personal_information: Optional[PersonalInfoItem] = Field(None, description='Extracted personal and contact details.')
	education_history: List[EducationItem] = Field(default_factory=list, description='List of extracted educational qualifications.')
	work_experience_history: List[WorkExperienceItem] = Field(default_factory=list, description='List of extracted work experiences.')
	skills_summary: List[SkillItem] = Field(default_factory=list, description='List of extracted skills.')
	projects_showcase: List[ProjectItem] = Field(default_factory=list, description='List of extracted personal or academic projects.')
	certificates_and_courses: List[CertificateItem] = Field(default_factory=list, description='List of extracted certifications and courses.')
	interests_and_hobbies: List[InterestItem] = Field(default_factory=list, description='List of extracted interests and hobbies.')
	...
	other_sections_data: Dict[str, List[str]] = Field(
		default_factory=dict, description='Data from other identified CV sections not fitting standard categories, stored as raw text blocks per section.'
	)

	cv_summary: Optional[str] = Field(None, description='A brief, LLM-generated summary of the entire CV.')
	extracted_keywords: List[str] = Field(default_factory=list, description='List of general keywords extracted from the CV content.')

	inferred_characteristics: List[InferredCharacteristicItem] = Field(default_factory=list, description='Inferred characteristics, potential roles, strengths, and insights about the candidate.')

	llm_token_usage: Optional[Dict[str, int]] = Field(
		None, description="Information about the number of tokens used during LLM processing (e.g., {'input_tokens': 500, 'output_tokens': 1500, 'total_tokens': 2000})."
	)
...
class CVState(TypedDict):
	...
	personal_info_item: Optional[PersonalInfoItem]  # Changed to single item, assuming one person per CV
	education_items: Optional[List[EducationItem]]
	work_experience_items: Optional[List[WorkExperienceItem]]
	skill_items: Optional[List[SkillItem]]
	project_items: Optional[List[ProjectItem]]
	certificate_items: Optional[List[CertificateItem]]
	interest_items: Optional[List[InterestItem]]
	...
	inferred_characteristics: Optional[List[InferredCharacteristicItem]]
